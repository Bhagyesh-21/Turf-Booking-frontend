    ...
    const handlePayment = async (mode) => {
        ...
        try {
            ...
            if (error.response?.data?.message) {
                ...
                setErrorMessage(
                    <>
                        Failed to process payment: Turf is already booked for the selected date and time.
                        <br />
                        Please select a different time slot or choose another turf.
                    </>
                );
            }
        } finally {
            ...
            {turfName && date && time && price ? (
                <div className="booking-details">
                    ...
                    <div className="payment-options">
                        ...
                        <button onClick={() => handlePayment('online')} disabled={isProcessing}>
                            {isProcessing ? 'Processing...' : 'Online'}
                        </button>
                        <button onClick={() => handlePayment('offline')} disabled={isProcessing}>
                            {isProcessing ? 'Processing...' : 'Offline'}
                        </button>
                    </div>
                </div>
            ) : (
